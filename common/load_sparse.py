...
def load_network(
    path="/data/allen/v1_model/GLIF_network/network_dat.pkl",
    ...
    if (
        use_uniform_neuron_type
    ):  # remove diversity of neurons, keep just one GLIF3 model for E and one for I
        node_params = dict(
            V_th=np.zeros(n_node_types, np.float32),
            g=np.zeros(n_node_types, np.float32),
            E_L=np.zeros(n_node_types, np.float32),
        ...
        df = pd.read_csv(
            os.path.join(data_dir, "network/v1_node_types.csv"), delimiter=" "
        )
        ...
        for i, node_type in enumerate(d["nodes"]):
            ...
            if use_only_one_type:
                ...
                if df.iloc[i]["pop_name"].startswith("e"):
                    ...
                    raise ValueError(
                        "It is neither excitatory nor inhibitory; something is wrong with your file!"
                    )
...
def load_input(
    path="/data/allen/v1_model/GLIF_network/input_dat.pkl",
    start=0,
    duration=3000,
    dt=1,
...
def load_TD_input(
    path, network, n_inputs, targets, inter_area_min_delay, inter_area_max_delay, seed
):
    with open(path, "rb") as f:
        d = pkl.load(f)
    ...
    cons = [
        edge["target"].__len__() for edge in d[0][1]
    ]  # d[0] stimulus, d[1] background
    num_cons = sum(cons)
    con_prob = num_cons / (
        17400
        * (
            network["laminar_indices"]["L4e"].size
            + network["laminar_indices"]["L4i"].size
        )
    )
    ...
    for target in targets_pool:
        tmp = rd.choice(
            int(target.size * n_inputs),
            int(0.1 * con_prob * target.size * n_inputs),
            replace=False,
        )
        post_indices.extend(
            target[np.mod(tmp, target.size)] * 4 + rd.randint(0, 4, tmp.size)
        )
    ...
    indices, weights = sort_indices(indices, weights)
    delays = rd.randint(
        low=inter_area_min_delay, high=inter_area_max_delay, size=weights.shape
    )
...
def set_laminar_indices(df, h5_path, network, L2_neuron_ratio=0.5):
    ...
    for i, pop_name in enumerate(all_pop_names):
        for j, pp_name in enumerate(neuron_pop_id_to_name):
            if pop_name.startswith(pp_name):
                ...
                break
    # pop_name_indices: fine-grained pop name -> neuron indices (by neuron_pop_id_to_name)
    network["pop_name_indices"] = {
        name: np.where(rough_neuron_pop_names == j)[0]
        for j, name in enumerate(neuron_pop_id_to_name)
    }
    ...
    for i, layer_number in enumerate([23, 4, 5, 6]):
        network["laminar_indices"][f"L{layer_number}e"] = np.where(
            rough_neuron_pop_names == exc_ind[i]
        )[0]
    ...
    L23_neuorn_indices = np.hstack(
        (network["laminar_indices"]["L23e"], network["laminar_indices"]["L23i"])
    )

    L2_argindices = L23_argindices_sorted[
        : np.int64(L2_neuron_ratio * vertical_coordinates.size)
    ]
    ...
    L3_argindices = L23_argindices_sorted[
        np.int64(L2_neuron_ratio * vertical_coordinates.size) :
    ]
    ...
    # global excitatory / inhibitory indices
    # all_pop_names 中以 'e' 开头的是兴奋性，其余（以 'i' 开头）为抑制性
    is_exc = np.array([name.startswith("e") for name in all_pop_names], dtype=bool)
    network["laminar_indices"]["e"] = np.where(is_exc)[0]
    network["laminar_indices"]["i"] = np.where(~is_exc)[0]
...
def load_billeh(
    ...
    # 计算 readout_neuron_sel：选出距离中心 (x^2 + z^2) 最近的 n_output 个 L5e 神经元
    readout_neuron_sel = np.zeros(network["n_nodes"], dtype=bool)
    l5e_neuron_indices = np.where(l5e_neuron_sel)[0]
    if l5e_neuron_indices.size > 0:
        dist2 = np.square(x[l5e_neuron_indices]) + np.square(z[l5e_neuron_indices])
        k = min(n_output, l5e_neuron_indices.size)
        nearest_idx = l5e_neuron_indices[np.argsort(dist2)[:k]]
        readout_neuron_sel[nearest_idx] = True
    network["readout_neuron_sel"] = np.asarray(readout_neuron_sel)
    print(
        f"> Number of readout L5e Neurons (closest in x-z): "
        f"{int(np.sum(readout_neuron_sel))}"
    )
    ...
    if localized_readout:
        try:
            ...
            print(
                "Warning: Small neuronal volume, not all readout populations available"
            )
        ...
        for i in range(15):
            network[f"localized_readout_neuron_ids_{i}"] = readout_neurons_random[i, :][
                None, :
            ]
...
if __name__ == "__main__":
    ...
    if hostname.count("scherr-pc") > 0:
        ...
        _data_dir = "/mnt/zpool-febdash/dataset/GLIF_V1_network"
