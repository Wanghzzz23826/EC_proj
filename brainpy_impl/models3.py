...
class AlphaCUBA(_StateSnapshotMixin, bp.Projection):
    ...
    def _state_items(self):
        items = []
        syn = getattr(self.proj, "syn", None)
        if syn is not None and hasattr(syn, "_state_items"):
            for path, var in syn._state_items():
                items.append((_state_key("syn") + path, var))
        return items

    def reset_state(self, batch_size=None):
        # Explicit child reset is required for stable TF/BP alignment.
        # Some Projection traversals do not reset internal syn states in Python wrappers.
        if hasattr(self.proj, "syn") and hasattr(self.proj.syn, "reset_state"):
            self.proj.syn.reset_state(batch_size)
        if hasattr(self.proj, "out") and hasattr(self.proj.out, "reset_state"):
            self.proj.out.reset_state(batch_size)
    ...
    **xargs,
    ...
    for p in syn_params_list:
        ...
        if delay_filter is None or delay_steps in delay_filter:
            # SynapseParams.delays are "delay-1" (k).
            # BrainPy delay registration has an extra scheduling offset that depends on
            # projection/update order. Keep it configurable for strict TF alignment.
            delay_steps = int(delay_steps) + int(delay_register_offset)
            ret.append(
                cls(
                    conn=conn,
                    weight=weight,
...
class BillehColumn(_StateSnapshotMixin, bp.DynSysGroup):
    ...
    def reset_state(self, batch_size=None):
        ...
        # Explicitly reset the shared pre-neuron delay ring buffer.
