...
_LGN_MODEL_MODULE = None


def _require_lgn_model():
    global _LGN_MODEL_MODULE
    if _LGN_MODEL_MODULE is not None:
        return _LGN_MODEL_MODULE

    errors = []
    for module_name in (
        "tensorflow_impl.lgn_model",
        "lgn_model",
    ):
        try:
            _LGN_MODEL_MODULE = importlib.import_module(module_name)
            return _LGN_MODEL_MODULE
    ...
    raise ImportError(
        "Could not import legacy LGN module needed by brainpy_impl.stim_dataset.\n"
        f"Tried:\n{joined}"
    )
...
def generate_data_set_continuing(path='image_outputs', batch_size=1, seq_len=1000, examples_in_epoch=50,
                                 ...
                                 current_input=True, pre_chunks=4, resp_chunks=1,  shared_seqs=None,shared_changes=None):
    if path.split('.')[-1] == 'pkl':
    ...
    if shared_seqs is None or shared_changes is None:
        raise ValueError("shared_seqs and shared_changes must be provided")

    def gen_seq_override(_):
        # return shared sequences as tf.Constant
        return (
            tf.constant(shared_seqs, dtype=tf.int32),
            tf.constant(shared_changes, dtype=tf.int32)
        )
    ...
    data_set = tf.data.Dataset.from_tensor_slices([0]).map(gen_seq_override)
    data_set = (
        data_set.unbatch()
                 .map(concat)
                 .unbatch()
                 .batch(int(seq_len / 50))
                 .map(l1)
                 .map(l2)
    )
...
def _vcd_orientation_keys(orientation_values=None):
    if orientation_values is None:
        values = np.arange(1200, 1501, dtype=np.int32)
    else:
        arr = np.asarray(orientation_values, dtype=np.float32).reshape(-1)
        if arr.size == 0:
            raise ValueError('orientation_values must be non-empty.')
        values = np.unique(np.rint(arr * 10.0).astype(np.int32))
    return values


def _vcd_orientation_from_key(key):
    return float(int(key)) / 10.0
...
def _resolve_vcd_phase_values(num_phases=8, phase_values=None):
    if phase_values is None:
        n = int(num_phases)
        if n <= 0:
            raise ValueError(f'num_phases must be positive, got {n!r}.')
        return np.linspace(0.0, 180.0, n, endpoint=False, dtype=np.float32)

    arr = np.asarray(phase_values, dtype=np.float32).reshape(-1)
    if arr.size == 0:
        raise ValueError('phase_values must be non-empty when provided.')
    arr = np.mod(arr, 180.0).astype(np.float32, copy=False)
    return np.unique(arr)


def _phase_bank_token(phase_values):
    arr = np.asarray(phase_values, dtype=np.float32).reshape(-1)
    digest = hashlib.sha1(arr.tobytes()).hexdigest()[:10]
    return f'ph{arr.size}_{digest}'


def _resolve_vcd_lgn_cache_bucket(lgn_cache_dir, *, data_dir, intensity, im_slice, pre_delay, post_delay, phase_values, cache_kind='rates'):
    root = Path(lgn_cache_dir).expanduser().resolve()
    data_tag = Path(os.path.abspath(data_dir)).name
    phase_tag = _phase_bank_token(phase_values)
    cache_kind = str(cache_kind)
    if cache_kind not in ('rates', 'input'):
        raise ValueError(f"Unsupported VCD cache_kind={cache_kind!r}. Expected 'rates' or 'input'.")
    suffix = '' if cache_kind == 'rates' else f'_{cache_kind}'
    bucket = root / data_tag / (
        f"i{_cache_token(intensity)}_im{int(im_slice)}_pre{int(pre_delay)}_"
        f"post{int(post_delay)}_{phase_tag}{suffix}"
    )
    return bucket


def _resolve_vcd_lgn_cache_file(bucket: Path, orientation, phase_idx):
    ori_key = int(np.rint(float(orientation) * 10.0))
    return bucket / f"ori_{ori_key}_phase_{int(phase_idx):03d}.npy"


def _build_vcd_single_stimulus_movie(orientation, *, intensity, im_slice, pre_delay, post_delay, phase):
    tiled_img = make_drifting_grating_stimulus(
        moving_flag=False,
        image_duration=im_slice,
        cpd=0.05,
        temporal_f=2,
    ...
    tiled_img = ((tiled_img[..., None]).astype(np.float32) - 0.5) * float(intensity) / 0.5
    zeros_pre = np.zeros((int(pre_delay), tiled_img.shape[1], tiled_img.shape[2], 1), dtype=np.float32)
    zeros_post = np.zeros((int(post_delay), tiled_img.shape[1], tiled_img.shape[2], 1), dtype=np.float32)
    return np.concatenate((zeros_pre, tiled_img, zeros_post), axis=0)


def _compute_vcd_single_stimulus_rates(lgn, orientation, *, intensity, im_slice, pre_delay, post_delay, phase):
    movie = _build_vcd_single_stimulus_movie(
        orientation,
        intensity=intensity,
        im_slice=im_slice,
        pre_delay=pre_delay,
    ...
    firing_rates = lgn.firing_rates_from_spatial(*spatial)
    return np.asarray(firing_rates, dtype=np.float32)


def _convert_rates_to_vcd_input(firing_rates):
    firing_rates = np.asarray(firing_rates, dtype=np.float32)
    _p = 1.0 - np.exp(-firing_rates / 1000.0)
    return (_p * 1.3).astype(np.float32, copy=False)


def _compute_vcd_single_stimulus_input(lgn, orientation, *, intensity, im_slice, pre_delay, post_delay, phase):
    firing_rates = _compute_vcd_single_stimulus_rates(
        lgn,
        orientation,
        intensity=intensity,
        im_slice=im_slice,
    ...
    return _convert_rates_to_vcd_input(firing_rates)


def ensure_vcd_lgn_cache(
    *,
    data_dir,
    lgn_cache_dir,
    intensity,
    ...
    if lgn_cache_dir is None:
        raise ValueError('lgn_cache_dir must be provided to build the VCD LGN cache.')

    cache_kind = str(cache_kind)
    if cache_kind not in ('rates', 'input'):
        raise ValueError(f"Unsupported VCD cache_kind={cache_kind!r}. Expected 'rates' or 'input'.")
    if cache_kind == 'input' and not current_input:
        raise ValueError("cache_kind='input' only matches current_input=True because it stores the final post-LGN _z input.")

    phase_bank = _resolve_vcd_phase_values(num_phases=num_phases, phase_values=phase_values)
    bucket = _resolve_vcd_lgn_cache_bucket(
        lgn_cache_dir,
        data_dir=data_dir,
        intensity=intensity,
        im_slice=im_slice,
        pre_delay=pre_delay,
        post_delay=post_delay,
        phase_values=phase_bank,
        cache_kind=cache_kind,
    )
    bucket.mkdir(parents=True, exist_ok=True)

    orientation_keys = _vcd_orientation_keys(orientation_values)
    meta = {
        'data_dir': os.path.abspath(data_dir),
        'intensity': float(intensity),
        'im_slice': int(im_slice),
        'pre_delay': int(pre_delay),
        'post_delay': int(post_delay),
        'orientation_keys': [int(v) for v in orientation_keys.tolist()],
        'num_phases': int(phase_bank.size),
        'phase_values': [float(v) for v in phase_bank.tolist()],
        'dtype': 'float16',
        'stimulus_len': int(pre_delay) + int(im_slice) + int(post_delay),
        'cache_kind': cache_kind,
    }
    (bucket / 'meta.json').write_text(json.dumps(meta, indent=2))
    ...
    with tf.device('/CPU:0'):
        for key in orientation_keys:
            orientation = _vcd_orientation_from_key(key)
            for phase_idx, phase in enumerate(phase_bank):
                cache_file = _resolve_vcd_lgn_cache_file(bucket, orientation, phase_idx)
                if cache_file.exists() and not overwrite:
                    existing += 1
                    continue
                if lgn is None:
                    lgn = _require_lgn_model().LGN(data_dir=data_dir)
                if cache_kind == 'input':
                    cache_arr = _compute_vcd_single_stimulus_input(
                        lgn,
                        orientation,
                        intensity=intensity,
                        im_slice=im_slice,
                    ...
                    cache_arr = _compute_vcd_single_stimulus_rates(
                        lgn,
                        orientation,
                        intensity=intensity,
                        im_slice=im_slice,
                ...
                np.save(cache_file, cache_arr.astype(np.float16, copy=False))
                created += 1
                if verbose:
                    print(
                        f'[VCD LGN cache:{cache_kind}] cached orientation={orientation:.1f} '
                        f'phase_idx={phase_idx} phase={float(phase):.4f} -> {cache_file.name}'
                    )

    return {
        'cache_dir': str(bucket),
        'num_orientations': int(len(orientation_keys)),
        'num_phases': int(phase_bank.size),
        'num_created': int(created),
        'num_existing': int(existing),
        'phase_values': [float(v) for v in phase_bank.tolist()],
        'cache_kind': cache_kind,
    }
...
def generate_VCD_orientation(intensity=2, im_slice=100, pre_delay=50, post_delay=150, p_reappear=0.5, pairs_in_epoch=50,
                                                        batch_size=2, current_input=True, data_dir='GLIF_network',
                                                        lgn_cache_dir=None, lgn_cache_num_phases=8, lgn_cache_phase_values=None, lgn_cache_kind='rates'):
    ...
    orientations = np.array(orientations, dtype=np.float32).reshape(batch_size, -1, 2)
    changes = changes.reshape(batch_size, -1, 2)
    # re-arrange for batches
    temp_ori = []
    temp_cha = []
    ...
    orientations = np.concatenate(temp_ori, axis=1).reshape(-1, 2).astype(np.float32, copy=False)
    changes = np.concatenate(temp_cha, axis=1).reshape(-1, 2)
    ...
    if lgn_cache_dir is None:
        ...
        def _g():
            for change, orientaion in zip(changes, orientations):
                ...
                if current_input:
                    ...
                    _z = tf.cast(tf.random.uniform(tf.shape(_p)) < _p, tf.float32)
                ground_truth = tf.cast(change, tf.float32)
                label = tf.concat([tf.zeros(pre_chunks)] + [ground_truth[0]*tf.ones(resp_chunks)] + [tf.zeros(post_chunks)] +                    [tf.zeros(pre_chunks)] + [ground_truth[1]*tf.ones(resp_chunks)] + [tf.zeros(post_chunks)],axis=0)
                weight = tf.concat([0.0*tf.ones(pre_chunks)] + [tf.ones(resp_chunks)] + [0.0*tf.ones(post_chunks)] +                                   [0.0*tf.ones(pre_chunks)] + [tf.ones(resp_chunks)] + [0.0*tf.ones(post_chunks)], axis=0)
                # for plotting, label the image when it holds on
        ...
        cache_info = ensure_vcd_lgn_cache(
            data_dir=data_dir,
            lgn_cache_dir=lgn_cache_dir,
            intensity=intensity,
            im_slice=im_slice,
            pre_delay=pre_delay,
            post_delay=post_delay,
            orientation_values=np.unique(orientations),
            num_phases=lgn_cache_num_phases,
            phase_values=lgn_cache_phase_values,
            cache_kind=lgn_cache_kind,
            current_input=current_input,
            overwrite=False,
            verbose=False,
        )
        cache_bucket = Path(cache_info['cache_dir'])
        phase_values = np.asarray(cache_info['phase_values'], dtype=np.float32)
        cache_kind = str(cache_info.get('cache_kind', lgn_cache_kind))
        cached_values = {}

        def sample_phase_index():
            if phase_values.size == 1:
                return 0
            return int(np.random.randint(phase_values.size))

        def get_cached_value(orientation, phase_idx):
            key = (int(np.rint(float(orientation) * 10.0)), int(phase_idx))
            arr = cached_values.get(key)
            if arr is None:
                arr = np.load(_resolve_vcd_lgn_cache_file(cache_bucket, orientation, phase_idx), mmap_mode='r')
                cached_values[key] = arr
            return arr

        def _g():
            for change, orientaion in zip(changes, orientations):
                phase_idx1 = sample_phase_index()
                phase_idx2 = sample_phase_index()
                value1 = get_cached_value(orientaion[0], phase_idx1).astype(np.float32, copy=False)
                value2 = get_cached_value(orientaion[1], phase_idx2).astype(np.float32, copy=False)
                if cache_kind == 'input':
                    if not current_input:
                        raise ValueError("cache_kind='input' only matches current_input=True in generate_VCD_orientation().")
                    _z = np.concatenate((value1, value2), axis=0).astype(np.float32, copy=False)
                else:
                    firing_rates = np.concatenate((value1, value2), axis=0)
                    if current_input:
                        _z = _convert_rates_to_vcd_input(firing_rates)
                    else:
                        _p = 1 - np.exp(-firing_rates / 1000.0)
                        _z = (np.random.uniform(size=_p.shape) < _p).astype(np.float32)

                ground_truth = np.asarray(change, dtype=np.int32)
                label = np.concatenate([
                    np.zeros(pre_chunks, dtype=np.int32),
                    np.full(resp_chunks, ground_truth[0], dtype=np.int32),
                    np.zeros(post_chunks, dtype=np.int32),
                    np.zeros(pre_chunks, dtype=np.int32),
                    np.full(resp_chunks, ground_truth[1], dtype=np.int32),
                    np.zeros(post_chunks, dtype=np.int32),
                ], axis=0)
                weight = np.concatenate([
                    np.zeros(pre_chunks, dtype=np.float32),
                    np.ones(resp_chunks, dtype=np.float32),
                    np.zeros(post_chunks, dtype=np.float32),
                    np.zeros(pre_chunks, dtype=np.float32),
                    np.ones(resp_chunks, dtype=np.float32),
                    np.zeros(post_chunks, dtype=np.float32),
                ], axis=0)
                image_label1 = np.concatenate([
                    np.zeros(int(pre_delay / chunk_size), dtype=np.float32),
                    np.full(int(im_slice / chunk_size), orientaion[0], dtype=np.float32),
                    np.zeros(int(post_delay / chunk_size), dtype=np.float32),
                ], axis=0)
                image_label2 = np.concatenate([
                    np.zeros(int(pre_delay / chunk_size), dtype=np.float32),
                    np.full(int(im_slice / chunk_size), orientaion[1], dtype=np.float32),
                    np.zeros(int(post_delay / chunk_size), dtype=np.float32),
                ], axis=0)
                image_labels = np.concatenate([image_label1, image_label2], axis=0)
                yield _z, label, image_labels, weight
