...
#空间卷积相关 每个神经元用自己的temporal kernel对自己的空间响应做FIR滤波
# input : all_spatial_responses: (T, N) temporal_kernels: (K, N)
# output：(T, N)
def temporal_filter(all_spatial_responses, temporal_kernels):
    tr_spatial_responses = tf.pad(
        all_spatial_responses[None, :, None, :],
        ((0, 0), (temporal_kernels.shape[0] - 1, 0), (0, 0), (0, 0))) #(T, N)-->(1, T+K-1, 1, N)

    filtered_output = tf.nn.depthwise_conv2d(
        tr_spatial_responses, temporal_kernels[:, None, :, None], strides=[1, 1, 1, 1], padding='VALID')[0, :, 0]#2d:neuron*kernel
    return filtered_output

#pad中tfpad和bp的pad都是在前面补零使之因果，输出shape都是（T，N） 

#hard ReLU LGN的firing rate 不是线性的
def transfer_function(arg__a, dtype=tf.float32):
    _h = tf.cast(arg__a >= 0, dtype)
    return _h * arg__a

# 空间对齐
#  input :x, y: (N,)，浮点坐标  convolved_movie: (T, H, W)
#  output:(T, N)
...
_LGN_REQUIRED_COLS = [
    'spatial_size',
    'kpeaks_dom_0', 'kpeaks_dom_1',
    'weight_dom_0', 'weight_dom_1',
    'delay_dom_0', 'delay_dom_1',
]
_LGN_COMPOSITE_REQUIRED_COLS = [
    'kpeaks_non_dom_0', 'kpeaks_non_dom_1',
    'weight_non_dom_0', 'weight_non_dom_1',
    'delay_non_dom_0', 'delay_non_dom_1',
    'tuning_angle', 'sf_sep',
]


def _lgn_param_issues(df):
    issues = []
    for col in _LGN_REQUIRED_COLS:
    ...
    if 'model_id' not in df.columns:
        issues.append('missing column: model_id')
        return issues

    model_ids = df['model_id'].astype(str)
    composite = model_ids.str.contains('ON') & model_ids.str.contains('OFF')
    for col in _LGN_COMPOSITE_REQUIRED_COLS:
        if col not in df.columns:
            issues.append(f'missing column: {col}')
            continue
        if df.loc[composite, col].isna().any():
            issues.append(f'NaN values in {col} for composite cells')

    for col in ('delay_dom_0', 'delay_dom_1', 'delay_non_dom_0', 'delay_non_dom_1'):
        if col not in df.columns:
            continue
...
def _fill_missing_lgn_params(df, ref_df):
    out = df.copy()
    cols_to_fill = _LGN_REQUIRED_COLS + _LGN_COMPOSITE_REQUIRED_COLS

    if 'id' in out.columns and 'id' in ref_df.columns:
        ref = ref_df.drop_duplicates(subset=['id']).set_index('id')
        for col in cols_to_fill:
            if col not in ref.columns:
                continue
            if col not in out.columns:
                out[col] = np.nan
            ref_vals = out['id'].map(ref[col])
            out[col] = out[col].where(out[col].notna(), ref_vals)
    elif len(out) == len(ref_df):
        for col in cols_to_fill:
            if col not in ref_df.columns:
                continue
            if col not in out.columns:
                out[col] = np.nan
            out[col] = out[col].where(out[col].notna(), ref_df[col].to_numpy())

    return out


def _repair_lgn_units_info(df, data_dir, cache_path):
    issues = _lgn_param_issues(df)
    if not issues:
        return df

    candidate_paths = [
        os.path.join(data_dir, 'lgn_full_col_cells_3.csv'),
        os.path.join(data_dir, 'tf_data', 'lgn_full_col_cells_3.csv'),
    ]
    for cand in candidate_paths:
        if not os.path.exists(cand):
            continue
        ref_df = pd.read_csv(cand, sep=r'\s+')
        repaired = _fill_missing_lgn_params(df, ref_df)
        repaired_issues = _lgn_param_issues(repaired)
        if not repaired_issues:
            if cache_path is not None:
                repaired.to_csv(cache_path, index=False, sep=' ', na_rep='NaN')
            print(f'Repaired LGN parameter table using reference file: {cand}')
            return repaired

    issue_text = '; '.join(issues)
    raise ValueError(
        'LGN parameter table is incomplete and could not be repaired. '
        f'Issues: {issue_text}. '
        'Provide a complete lgn_full_col_cells file with temporal/spatial parameters.'
    )
...
class LGN(object):
    def __init__(self, row_size=80, col_size=120, data_dir='GLIF_network', n_input=None, dtype=tf.float32):
        ...
        if os.path.exists(lgn_data_path):
            d = pd.read_csv(lgn_data_path, sep=r'\s+')
        else:
        ...
        d = _repair_lgn_units_info(d, data_dir_rel, lgn_data_path)
