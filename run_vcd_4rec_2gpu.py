"""
Two-GPU Temporal-LGN + 4-receptor-background variant of run_vcd.py.
...
It splits the RSRP/ES population axis across local GPUs with jax.pmap().
"""
...
os.environ.setdefault(
    ...
    os.environ.get(
        "RUN_VCD_4REC_2GPU_CUDA_VISIBLE_DEVICES",
        os.environ.get(
            "RUN_VCD_4REC_CUDA_VISIBLE_DEVICES",
            os.environ.get("RUN_VCD_LGN_CUDA_VISIBLE_DEVICES", "0,1"),
        ),
    ),
)
os.environ.setdefault(
...
def main():
    ...
    conf = OmegaConf.merge(
         {
            ...
            "pop_size": 1000,
            "total_pop_size": 1000,
            # Parallelize the population axis across local GPUs.
            # With two 24GB cards, keep total_pop_size the same and make pop_size divisible by 2.
            "pmap_pop_axis": True,
            "pmap_num_devices": 2,
    ...
    pmap_pop_axis = bool(getattr(conf, "pmap_pop_axis", True))
    pmap_num_devices = int(getattr(conf, "pmap_num_devices", 2))
    pmap_devices = tuple(jax.local_devices()[:pmap_num_devices]) if pmap_pop_axis else ()
    if pmap_pop_axis:
        if len(pmap_devices) != pmap_num_devices:
            raise RuntimeError(
                f"pmap_pop_axis=True requires {pmap_num_devices} local JAX devices, "
                f"but only found {len(jax.local_devices())}: {jax.local_devices()}. "
                "Set CUDA_VISIBLE_DEVICES/RUN_VCD_4REC_2GPU_CUDA_VISIBLE_DEVICES before import."
            )
        if int(conf.pop_size) % pmap_num_devices != 0:
            raise ValueError(
                f"pop_size={conf.pop_size} must be divisible by pmap_num_devices={pmap_num_devices}."
            )
        print(
            f"[2GPU] pmap population axis across {pmap_num_devices} devices: "
            f"{[str(d) for d in pmap_devices]}, per_device_pop={int(conf.pop_size) // pmap_num_devices}"
        )
    ...
    def train(dataset_iterable, opt_state, optim_cls, weights, params, es_params, es_opt_state, es_opt_cls, key_local):
        ...
        for epoch in range(conf.n_epoch):
            for batch in tqdm(dataset_iterable, desc=f"epoch {epoch + 1}/{conf.n_epoch}"):
                ...
                if pmap_pop_axis:
                    per_device_pop = int(conf.pop_size) // pmap_num_devices

                    def _shard_pop_axis(tree):
                        return jax.tree_util.tree_map(
                            lambda x: x.reshape((pmap_num_devices, per_device_pop, *x.shape[1:])),
                            tree,
                        )

                    mask_weights_sharded = _shard_pop_axis(mask_weights)

                    if es_samples is None:
                        def _run_pop_shard(w_shard):
                            return jax.vmap(
                        ...
                        logits, sp_rate_pop = jax.pmap(
                            _run_pop_shard,
                            in_axes=0,
                            devices=pmap_devices,
                        )(mask_weights_sharded)
                    else:
                        es_samples_sharded = _shard_pop_axis(es_samples)

                        def _run_pop_shard(w_shard, e_shard):
                            return jax.vmap(
                        ...
                        logits, sp_rate_pop = jax.pmap(
                            ...
                            in_axes=(0, 0),
                            devices=pmap_devices,
                        )(mask_weights_sharded, es_samples_sharded)
                elif es_samples is None:
